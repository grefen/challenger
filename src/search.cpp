...
/// Search::think() is the external interface to Challenger's search, and is
...
moves_loop: // When in check and at SpNode search starts from here
  ...
  bool allows(const Position& pos, Move first, Move second) {
	...
	Bitboard m1att = pos.attacks_from(pos.piece_on(m1to), m1to, pos.pieces() ^ m2from, pos.occupied_rl90^square_rotate_l90_bb(m2from));
    if (m1att & m2to)
        return true;
  ...
  bool refutes(const Position& pos, Move first, Move second) {
            ...
            || type_of(pos.piece_on(m2from)) == KING))
    {
		...
		Bitboard occl90 = pos.occupied_rl90^square_rotate_l90_bb(m1from)^square_rotate_l90_bb(m1to)^square_rotate_l90_bb(m2from);
        Piece pc = pos.piece_on(m1from);

        // The moved piece attacks the square 'tto' ?
        if (pos.attacks_from(pc, m1to, occ, occl90) & m2to)
            return true;
        ...
        if (unlikely(xray) && (xray & ~pos.attacks_from<ROOK>(m2to)) && (xray & ~pos.attacks_from<CANNON>(m2to)))
            return true;
    }
